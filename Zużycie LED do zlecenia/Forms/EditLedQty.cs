...
namespace Zużycie_LED_do_zlecenia
{
    public partial class EditLedQty : Form
    {
        ...
        private string reelsCurrentOrderNo = "";
        private string binName = "";
        private int reelsCurrentQty = 0;
        private string[] superUsers = new string[] { "piotr.dabrowski", "tomasz.jurkin", "katarzyna.kustra" };
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (newLedQty >= 0)
            {
                if (newLedQty > 30000)
                {
                    MessageBox.Show("Maksymalna dopuszczalna wartość to 30000");

                    return;
                }
                if (newLedQty < reelsCurrentQty || superUsers.Contains(Environment.UserName))
                {
